...
public class InvertedIndex {
  ...
  //Weight for initial query
  public float alpha = 0;

  //Weight for relevant documents
  public float beta = 0;

  //Weight for irrelevant documents
   ...
   * 
   * 
   * @param pseudofeedback  Whether pseudo relevance feedback should be used
   * @param top_k The number of documents used for pseudo relevance feedback
   * @param alpha  The value of alpha in Ide_regular method
   * @param beta  The value of beta in Ide_regular method
   * @param gamma  The value of gamma in Ide_regular method
   */
 ...
 // Constructor for pseudofeedback experiment
  public InvertedIndex(File dirFile, short docType, boolean stem, boolean feedback,
                          boolean pseudofeedback, int top_k, float alpha, float beta, float gamma) {
    this.dirFile = dirFile;
    this.docType = docType;
    this.stem = stem;
    this.feedback = feedback;
  ...
  protected void indexDocuments() {
    ...
    //docType : DocumentIterator.TYPE_HTML / DocumentIterator.TYPE_TEXT
    ...
    while (docIter.hasMoreDocuments()) {
      //doc : HTMLFileDocument object or TEXTFileDocument object
  ...
  protected void indexDocument(FileDocument doc, HashMapVector vector) {
    ...
    //System.out.print("vector size : " + vector.entrySet().size() + ", ");
  ...
  public Retrieval[] retrieve(String input) {
    //input 예시 : What are the effects of calcium on the physical properties of mucus from CF patients?
    return retrieve(new TextStringDocument(input, stem, false));
  }
   ...
   */
  public void processQueries(boolean phrase) {
    ...
    do {
      ...
      //stem : false
      HashMapVector queryVector = (new TextStringDocument(query, stem, phrase)).hashMapVector();

      for (Map.Entry<String, Weight> entry : queryVector.hashMap.entrySet()) {
  ...
  public static void main(String[] args) {
    ...
    for (int i = 0; i < args.length - 1; i++) {
      ...
      if (flag.equals("-html"))
        ...
        top_k = Integer.parseInt(args[i+1]);
        pseudofeedback = true;
        i += 1;
      }
        ...
        beta = Float.parseFloat(args[i+2]);
        gamma = Float.parseFloat(args[i+3]);
        i += 3;
      }
    ...
    if(pseudofeedback){
      index = new InvertedIndex(new File(dirName), docType, stem, feedback, pseudofeedback, top_k, alpha, beta, gamma);
    }
    else{
      index = new InvertedIndex(new File(dirName), docType, stem, feedback);
    }
    // index.print();
    // Interactively process queries to this index.
    // Since VSR should process one-word phrases, the 'phrase' parameter of processQueries function is false.
    index.processQueries(false);
  }
}