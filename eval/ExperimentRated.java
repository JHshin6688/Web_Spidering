...
public class ExperimentRated extends Experiment {
  ...
  public ExperimentRated(File corpusDir, File queryFile, File outFile, short docType, boolean stem
      ...
      throws IOException {
      super(corpusDir, queryFile, outFile, docType, stem, pseudofeedback, top_k, alpha, beta, gamma);
  }
  ...
  // For processQuery function, this one is called 
  boolean processQuery(BufferedReader in) throws IOException {
    ...
    Retrieval[] retrievals_new;

    HashMapVector new_Query;                            

    //Pseudo Relevance Feedback is executed is 'psuedofeedback' is true
    if(this.pseudofeedback){
      Document query_doc = new TextStringDocument(query, this.stem, false);
      HashMapVector vec = query_doc.hashMapVector();

      Feedback pseudo = new Feedback(vec, retrievals, index, this.top_k
                                    , this.alpha, this.beta, this.gamma);
      
      new_Query = pseudo.newQuery_pseudo();
      retrievals = index.retrieve(new_Query);
    }
  ...
  public static void main(String[] args) throws IOException {
    ...
    for (int i = 0; i < args.length - 3; i++) {
      ...
      if (flag.equals("-html"))
        ...
        // Parse pseudo relevance feedback
       ...
       // Parse pseudo relevance parameters
    ...
    ExperimentRated exper = new ExperimentRated(new File(corpusDir), new File(queryFile),
        new File(outFile), docType, stem, pseudofeedback, top_k, alpha, beta, gamma);
